...
export default function CashflowDashboard() {
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col text-slate-800 dark:text-slate-200 font-sans selection:bg-emerald-100 dark:selection:bg-emerald-900/50 selection:text-emerald-900 dark:selection:text-emerald-100 transition-colors duration-300">
          ...
          <main className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-fade-in max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto justify-center">
              <button onClick={() => setShowLogin(true)} className="px-8 py-4 text-base font-semibold bg-emerald-600 hover:bg-emerald-700 text-white dark:text-slate-900 rounded-xl transition-all shadow-md shadow-emerald-600/20 flex items-center justify-center gap-2">
                Akses Dashboard
              </button>
              {deferredPrompt && (
                <button onClick={handleInstallClick} className="px-8 py-4 text-base font-semibold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-xl transition-all shadow-sm flex items-center justify-center gap-2">
                  <DownloadCloud className="w-5 h-5 text-emerald-600" /> Install App
                </button>
              )}
            </div>
          ...
          <footer className="py-6 text-center text-sm text-slate-400 border-t border-slate-200 dark:border-slate-800">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input type="email" required value={loginEmail} onChange={e => setLoginEmail(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all" placeholder="admin@pravenbali.com" />
            </div>
            <div>
              ...
              <input type="password" required value={loginPass} onChange={e => setLoginPass(e.target.value)}
                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all" placeholder="••••••••" />
            </div>
            ...
            <button type="submit" disabled={loginLoading}
              className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white dark:text-slate-900 font-bold rounded-xl text-sm transition-all shadow-md shadow-emerald-600/20 disabled:opacity-50">
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="w-10 h-10 border-4 border-slate-200 dark:border-slate-800 border-t-emerald-600 rounded-full animate-spin" />
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white pb-24 font-sans selection:bg-emerald-100 dark:selection:bg-emerald-900/50 selection:text-emerald-900 dark:selection:text-emerald-100 transition-colors duration-300">
      ...
      <nav className="max-w-6xl mx-auto px-4 mt-6 overflow-x-auto hide-scrollbar">
        <div className="flex gap-1.5 p-1 min-w-max border-b border-slate-200 dark:border-slate-800">
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-t-xl transition-all whitespace-nowrap border-b-2 -mb-[3px] ${tab === t.key ? 'text-emerald-700 border-emerald-600 bg-emerald-50/50' : 'text-slate-500 dark:text-slate-400 border-transparent hover:text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:bg-slate-800/50'}`}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 mt-6">
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                <div key={i} className="glass-card p-5 relative overflow-hidden group">
                  ...
                  <p className={`text-2xl font-extrabold mt-2 tracking-tight text-slate-900 dark:text-white dark:text-slate-900`}>{fmtRp(card.value)}</p>
                </div>
              ))}
            </div>
            ...
            <div className={`glass-card p-5 flex items-center justify-between border-l-4 ${Math.abs(tbDebit - tbCredit) < 0.01 ? 'border-l-emerald-500' : 'border-l-rose-500'}`}>
              ...
              <div className="text-right text-xs space-y-1">
                <p className="text-slate-500 dark:text-slate-400">Σ Debit: <span className="font-bold text-slate-900 dark:text-white dark:text-slate-900">{fmtRp(tbDebit)}</span></p>
                <p className="text-slate-500 dark:text-slate-400">Σ Credit: <span className="font-bold text-slate-900 dark:text-white dark:text-slate-900">{fmtRp(tbCredit)}</span></p>
              </div>
            </div>
            ...
            <div className="glass-card p-0 overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between bg-slate-50 dark:bg-slate-950/50">
                <h3 className="text-sm font-bold text-slate-900 dark:text-white dark:text-slate-900">Transaksi Terbaru</h3>
              </div>
              <div className="p-5 space-y-4 max-h-[400px] overflow-y-auto">
                {transactions.length === 0 ? (
                  <p className="text-center text-sm text-slate-400 py-6">Belum ada transaksi.</p>
                ) : transactions.slice(0, 10).map(tx => (
                  <div key={tx.id} className="p-4 bg-white dark:bg-slate-900 dark:bg-slate-100 border border-slate-200 dark:border-slate-800 rounded-xl hover:border-slate-300 dark:border-slate-700 transition-colors shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-sm font-bold text-slate-900 dark:text-white dark:text-slate-900 mt-0.5 truncate">{tx.description}</p>
                          ...
                          {tx.journal_entries.map((je, i) => (
                            <div key={i} className="flex items-center justify-between text-xs">
                              ...
                              <div className="flex gap-4 font-mono">
                                <span className={je.debit > 0 ? 'text-slate-900 dark:text-white dark:text-slate-900 font-semibold' : 'text-slate-300'}>{je.debit > 0 ? fmtRp(je.debit) : '-'}</span>
        ...
        {tab === 'worksheet' && (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white dark:text-slate-900 flex items-center gap-2"><FileSpreadsheet className="w-5 h-5 text-emerald-600" /> Neraca Lajur (Worksheet)</h2>
            </div>
            <Worksheet />
          </div>
        )}
        ...
        {tab === 'neraca' && (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white dark:text-slate-900 flex items-center gap-2"><ClipboardList className="w-5 h-5 text-emerald-600" /> Neraca Saldo (Trial Balance)</h2>
              <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 dark:bg-slate-100 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-800">{new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
            </div>
            <div className="glass-card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {trialBalance.map(row => (
                      <tr key={row.account_code} className="hover:bg-slate-50 dark:bg-slate-950/80 transition-colors">
                        ...
                        <td className="px-5 py-3 text-slate-900 dark:text-white dark:text-slate-900 font-medium">{row.account_name}</td>
                        ...
                        <td className="px-5 py-3 text-right font-mono text-slate-900 dark:text-white dark:text-slate-900">{row.total_debit > 0 ? fmtRp(row.total_debit) : '-'}</td>
                        ...
                        <td className={`px-5 py-3 text-right font-mono font-bold ${row.ending_balance >= 0 ? 'text-slate-900 dark:text-white dark:text-slate-900' : 'text-rose-600'}`}>
                  ...
                  <tfoot>
                    <tr className="bg-slate-50 dark:bg-slate-950 border-t-2 border-slate-200 dark:border-slate-800">
                      <td colSpan={3} className="px-5 py-4 font-bold text-slate-900 dark:text-white dark:text-slate-900 text-sm">TOTAL</td>
                      <td className="px-5 py-4 text-right font-mono font-bold text-slate-900 dark:text-white dark:text-slate-900">{fmtRp(tbDebit)}</td>
                      <td className="px-5 py-4 text-right font-mono font-bold text-slate-900 dark:text-white dark:text-slate-900">{fmtRp(tbCredit)}</td>
                      <td className="px-5 py-4 text-right font-mono font-bold text-slate-900 dark:text-white dark:text-slate-900">{fmtRp(tbDebit - tbCredit)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
        ...
        {tab === 'ledger' && (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white dark:text-slate-900 flex items-center gap-2"><BookOpen className="w-5 h-5 text-emerald-600" /> Jurnal Umum (General Ledger)</h2>
              <button onClick={() => setShowModal(true)}
                ...
                <div key={tx.id} className="glass-card p-0 overflow-hidden">
                  <div className="px-5 py-3 bg-slate-50 dark:bg-slate-950/80 border-b border-slate-100 dark:border-slate-800/50 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white dark:text-slate-900 mt-0.5">{tx.description}</p>
                    </div>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {tx.journal_entries.map((je, i) => (
                          <tr key={i} className="group hover:bg-slate-50 dark:bg-slate-950/50 transition-colors">
                            ...
                            <td className="py-2 px-2 text-right font-mono font-medium text-slate-900 dark:text-white dark:text-slate-900">{je.debit > 0 ? fmtRp(je.debit) : ''}</td>
        ...
        {tab === 'adjusting' && (
          <div className="space-y-4 animate-fade-in">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white dark:text-slate-900 flex items-center gap-2"><Settings className="w-5 h-5 text-amber-600" /> Jurnal Penyesuaian</h2>
              <button onClick={() => setShowAdjModal(true)}
                className="flex items-center gap-1.5 px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white dark:text-slate-900 font-semibold rounded-lg text-xs transition-all shadow-sm shadow-amber-600/20">
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 dark:bg-slate-100/90 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 px-4 py-3 flex items-center justify-between z-40">
